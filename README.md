...
- для не принципиально важных сервисов (счетчики, лайки, комменты, репосты) и тех, для которых нельзя выдать неактуальные данные (сообщения, чаты), игнорируем
...
### outbox
- данные по статистике постов пишутся одновременно и в БД (Tarantool) и в брокер сообщений (Kafka) для подсчета
